...
function deserializeHeader(buffer: ArrayBuffer) {
  ...
  return [
    new RaceSimulateHeaderData({ maxLength: maxLength, version: version }),
    4 + maxLength,
  ];
}
...
function deserializeHorseFrame(buffer: ArrayBuffer, offset: number) {
  ...
  ] = horseFrameStruct.unpack_from(buffer, offset);
...
function deserializeFrame(
  ...
  const frame = new RaceSimulateFrameData({
    time: oneFloat.unpack_from(buffer, offset)[0],
  });
  offset += 4;
...
function deserializeHorseResult(buffer: ArrayBuffer, offset: number) {
  const [
    ...
    defeat,
    finishTimeRaw,
  ] = horseResultStruct.unpack_from(buffer, offset);
...
function deserialize(input: Uint8Array) {
  ...
  const [distanceDiffMax, horseNum, horseFrameSize, horseResultSize] =
    raceStruct.unpack_from(buffer, offset);
