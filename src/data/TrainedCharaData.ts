    ...
    if (skillId < 200000) {
      ...
      [1, 2, 3, 4].forEach((runningStyleTag) => {
        if (tagIds.has('10' + runningStyleTag.toString())) {
          runningStyleMultiplier = _.max([
            runningStyleMultiplier,
            properSkillMultiplier[properRunningStyles[runningStyleTag]],
          ])!;
        }
      });
      ...
      [1, 2, 3, 4].forEach((distanceTag) => {
        if (tagIds.has('20' + distanceTag.toString())) {
          distanceMultiplier = _.max([
            distanceMultiplier,
            properSkillMultiplier[properDistances[distanceTag]],
          ])!;
        }
      });
      ...
      [1, 2].forEach((groundTag) => {
        if (tagIds.has('50' + groundTag.toString())) {
          groundMultiplier = _.max([
            groundMultiplier,
            properSkillMultiplier[properGrounds[groundTag]],
          ])!;
        }
      });
...
const properSkillMultiplier: Record<number, number> = {
  1: 0.7,
  2: 0.8,
  3: 0.8,
  4: 0.8,
