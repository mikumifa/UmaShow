...
目前没有证据表明官方客户端会在后台自动等待十分钟。UmaAutoResearch 不再为重复的 `217/218` 设置倒计时等待，而是立即重建会话并继续当前计划。用户需要让出账号时，应先在 UmaShow 点击“暂停当前计划”；暂停会保留计划进度，同时停止 Worker 和自动重登。
...
| `217` | 登录或会话连接冲突 | 停止当前请求，显示连接冲突错误，点击确定后返回标题并重新执行 `StartSession` | 立即重建会话并继续原计划，不进入倒计时等待；需要让出会话时由用户显式暂停计划 |
| `218` | 已在其他设备登录 | 停止当前设备的会话操作，显示其他设备登录提示，点击确定后返回标题页 | 与 `217` 一样立即重登并继续原计划；显式暂停后停止自动重登，允许其他设备稳定登录 |
